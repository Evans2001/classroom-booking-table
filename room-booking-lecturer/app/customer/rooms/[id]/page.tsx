...
import { Building2, Users, Layers, AlertCircle, CalendarDays, CheckCircle2, ChevronLeft, Wrench, MonitorPlay, Wifi, Video } from "lucide-react";
...
export default function RoomDetailsPage() {
  ...
  return (
    <div className="space-y-6 pb-24">
      {/* Top Action Bar */}
      <Link href="/customer/rooms" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-slate-900 transition-colors">
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 bg-white/80 pb-4 pt-4 px-4 backdrop-blur-md border-t border-slate-100">
        <div className="mx-auto flex max-w-md gap-3">
          <Button asChild variant="outline" className="w-1/3 bg-white h-12 rounded-xl border-slate-200 shadow-sm text-slate-700 hover:text-action-danger hover:border-action-danger hover:bg-action-danger/5">
            <Link href={`/customer/issues/new?roomId=${room.id}`} className="flex flex-col items-center justify-center gap-0.5">
              <span className="text-sm">Report</span>
            </Link>
          </Button>
          <Button asChild className="flex-1 h-12 rounded-xl shadow-lg shadow-brand-primary/20">
            <Link href={`/customer/bookings/new?roomId=${room.id}`} className="flex items-center justify-center gap-2">
